...
doc = """
Labels and Turnarounds Punishment/Bonus Treatments
"""
...
class Group(BaseGroup):
    ...
    display_punish_bonus_result = models.StringField()
    ...
    def set_payoffs(self):
        ...
        for p in players:
            ...
            if p.participant.vars['condition'] == 'Punish' and self.subsession.round_number > \
                    Constants.num_roundsr / 2 and vote_sum > 2 and self.min != 4 and p.total_sentences == self.min:
                p.round_earnings = 0
                p.counter = 'A'
            elif p.participant.vars['condition'] == 'Bonus' and self.subsession.round_number > \
                    Constants.num_roundsr / 2 and self.min == 4:
                p.round_earnings = p.round_earnings + Constants.bonus
                p.first_p = p.first_p + Constants.bonus
                p.second_p = p.second_p + Constants.bonus
                p.third_p = p.third_p + Constants.bonus
                p.fourth_p = p.fourth_p + Constants.bonus
                p.counter = 'B'
    ...
    def show_result(self):
        ...
        if count_a > 0:
            display_punish_bonus_result = "The majority voted to punish the lowest contributor(s). They have " \
                                      "earned 0 points for this round."
            for p in players:
            ...
            display_punish_bonus_result = "All firm employees correctly typed 4 sentences. Each member of " \
                                      "your firm has earned a 200 point bonus."
        elif count_x > 0:
            display_punish_bonus_result = " "
        else:
            display_punish_bonus_result = "No changes have been made to any employee's earnings"
        self.display_punish_bonus_result = display_punish_bonus_result
